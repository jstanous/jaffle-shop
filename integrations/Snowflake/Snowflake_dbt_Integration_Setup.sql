...
Sections:
  ...
  2. Database Setup
     - Creates DBT (default dbt database) and DBT_DEV (development database).
...
GRANT USAGE ON DATABASE DBT TO ROLE DBT_ROLE;
GRANT CREATE SCHEMA ON DATABASE DBT TO ROLE DBT_ROLE;
GRANT MONITOR ON DATABASE DBT TO ROLE DBT_ROLE;
GRANT USAGE ON FUTURE SCHEMAS IN DATABASE DBT TO ROLE DBT_ROLE;
GRANT CREATE TABLE
     ,CREATE VIEW
     ,CREATE STAGE
     ,CREATE FILE FORMAT
   ON FUTURE SCHEMAS IN DATABASE DBT
   TO DBT_ROLE;
...
BEGIN
  schema_name := 'DBT_DEV.' || CURRENT_USER();
